...
export default class extends Super {
  ...
  static rule = {
    ...
    "handles.core:selection": {
            ...
            if (pnode) {
              ...
              return this.data_column > 1
                ? [
                    "cursor",
                    pnode,
                    { dir: "x", x: rect.width, y: 0, size: rect.height },
                  ]
                : [
                    "cursor",
                    pnode,
      ...
      move(p, dir, pos) {
        switch (dir) {
          case "top":
            return pos - this.data_column.val;
          case "bottom":
            return pos + this.data_column.val;
          case "left":
            return pos - 1;
    ...
    "handles.core:selection.scaler.right.move"(
      p,
      { move_x, delta_x, start_rect, current_rect },
    ) {
      if (this.data.length) {
        ...
        if (delta_x > 8 && move_x > 0 && next_node) {
          ...
          if (
            prev_rect.right - current_rect.left + next_rect.width / 2 <
          ...
          if (
            prev_rect.right - current_rect.left - prev_rect.width / 2 >
            current_rect.width + delta_x
          ) {
            this.data_column.val--;
          }
  ...
  init(data) {
    ...
    this.data_column = [
      this.data.length && this.data.every((n) => n.is(".json:number"))
        ? data.length
        : 1,
    ];
  }
  ...
  struct() {
    ...
    return frame([
      ...
      .into(this)
      .color(200)
      .style_on("sbracket-before", "sbracket-after").melem;
  }
