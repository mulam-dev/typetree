...
export default class extends Super {
  ...
  static rule = {
    ...
    "handles.core:layout": {
      "get-selection"(p, anchor_node, focus_node) {
        let anchor_entry_idx = this.data.findIndex((entry) =>
          entry.includes(anchor_node),
        );
        let focus_entry_idx = this.data.findIndex((entry) =>
          entry.includes(focus_node),
        );
        ...
        if (Math.abs(anchor_entry_idx - focus_entry_idx) > 1) {
          return this.$type["#core:selection"]({
            scope: this,
            anchor: [anchor_entry_idx, 0],
            focus: [focus_entry_idx, 2],
          });
        } else {
          ...
          return this.$type["#core:selection"]({
            scope: this,
            anchor: [anchor_entry_idx, anchor],
            focus: [focus_entry_idx, focus],
          });
        }
      },
    },
          ...
          if (this.data.length) {
            ...
            if (pentry) {
              ...
              return [
                "cursor",
                nentry[0],
                { dir: "y", x: 0, y: 0, size: vrect.right - krect.left },
              ];
            }
          } else {
  ...
  struct() {
    ...
    return frame([
      ...
      .into(this)
      .color(180, 0.5, 1.1)
      .style_on("hint-before").melem;
  }

  to_json() {
    return Object.fromEntries(
      this.data.map(([k, v]) => [k.to_json(), v.to_json()]),
    );
  }
